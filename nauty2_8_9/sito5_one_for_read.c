...
int eigensymmatrix(char * BUFOR)
{
    ...
    int k2 = n;
    {
        ...
        if(!((ceil(u)-u<1e-5) || (u-floor(u)<1e-5))){
            return 0;
        }
    } 
    return 1;
...
int main(int argc, char *argv[])
{
    ...
    while (counts[current] > 0) {
        ...
        int worker_threads = total_threads - 1;

        #pragma omp parallel default(none) shared(buffor, counts, compute, next, t, stdin, worker_threads)//shared - wspólne zmienne
        {
            #pragma omp sections 
            {
                #pragma omp section //jeden wątek odczytuje i kopiuje grafy do buforu następnego
                {
                ...
                #pragma omp section
                {
                    #pragma omp parallel for schedule(dynamic, 1) num_threads(worker_threads) shared(buffor, counts, compute)// inne wątki wykonują obliczenia. Dla jednego wątku - jeden graf
